...
class Psi_Thermwind(object):
  ...
  def __init__(
    ...
    if t1 is not None: 
        self.t1 = make_func(t1, self.z, 't1')
        self.t2 = make_func(t2, self.z, 't2')
  ...
  def Psit(self, nb=500):
    ...
    """
    # map overturning into isopycnal space:
    t1 = make_array(self.t1, self.z, 't1')
    t2 = make_array(self.t2, self.z, 't2')
    tmin = min(np.min(t1), np.min(t2))
    tmax = max(np.max(t1), np.max(t2))
    self.tgrid = np.linspace(tmin, tmax, nb)
    udydz = -(self.Psi[1:] - self.Psi[:-1])
    psit = 0. * self.tgrid
    tup_bot = t1[:-1].copy()
    tup_top = t1[1:].copy()
    idx = udydz < 0
    tup_bot[idx] = t2[:-1][idx]
    tup_top[idx] = t2[1:][idx]
    for i in range(0, len(self.tgrid)):
      mask = np.clip((tup_top - self.tgrid[i]) / (tup_top-tup_bot), 0., 1.)
      psit[i] = np.sum(mask * udydz)
    return psit

  def Psitz(self, nb=500):
    r"""
    Remap the overturning streamfunction onto the native isopycnal-depth space
    of the columns in the northern region and southern basin.
    ...
    # This does a linear interploation in b:
    return [
        np.interp(self.t1(self.z), self.tgrid, psit),
        np.interp(self.t2(self.z), self.tgrid, psit)
    ]
    # Ths instead first estimates the depth levels for the bgrid and then does linear interpolation in z
    # either has pros and cons depending on the situation...
    #z1_of_bgrid=np.interp(self.bgrid,self.b1(self.z),self.z)
...
# --- add inside class Psi_Thermwind ---
  ...
  def update_tracer(self, t1=None, t2=None):
    r"""
    Update the vertical tracer profiles from the southern basin and northern region.
