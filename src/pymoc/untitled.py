...
class Column(object):
  ...
  def solve_equi(self, wA):
    ...
    sol_init_b = np.zeros((2, np.size(self.z)))
    sol_init_q = np.zeros((2, np.size(self.z)))

      
    sol_init_b[0, :] = self.b
    sol_init_b[1, :] = self.bz
    res_b = integrate.solve_bvp(self.ode, self.bc, self.z, sol_init_b)
    # interpolate solution for b and db/dz onto original grid
    self.b = res_b.sol(self.z)[0, :]
    self.bz = res_b.sol(self.z)[1, :]

    sol_init_q[0, :] = self.q
    sol_init_q[1, :] = self.qz
    res_t = integrate.solve_bvp(self.ode, self.bc, self.z, sol_init_q)
    # interpolate solution for b and db/dz onto original grid
    self.q = res_q.sol(self.z)[0, :]
    self.qz = res_q.sol(self.z)[1, :]
  ...
  def Q_vertadvdiff(self, wA, dt, do_conv=False):
    r"""
  ...
  def Q_horadv(self, vdx_in, q_in, dt):
    r"""
    Carry out horizon buoyancy advection into the column model from an adjoining model,
    for the timestepping solution. This function implements an upwind advection scheme.
  ...
  def timestep(self, wA=0., dt=1., do_conv=False, vdx_in=None, b_in=None, q_in = None):
    r"""
    ...
    # do vertical advection and diffusion
    self.Q_vertadvdiff(wA=wA, dt=dt, do_conv=do_conv)
    
    if vdx_in is not None:
      # do horizontal advection: (optional)
      if q_in is not None:
        self.Q_horadv(vdx_in=vdx_in, q_in=q_in, dt=dt)
      else:
