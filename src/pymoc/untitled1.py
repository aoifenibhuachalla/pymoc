...
class SO_ML(object):
  ...
  def __init__(
    ...
    # Notice that the first and last gridpoints are BCs and should have no flux
      ...
      qurflux = 0, #  prescribed surface tracer flux (in m^2/s^3; input)
      q_rest = 0, # # surface tracer towards which we are restoring 
      qs=0.0,    # Surface tracer (input, output)
  ...
  def set_boundary_conditions(self, b_basin, Psi_b, q_basin, Psi_q):
    r"""
    ...
    if self.Psi_s[1] > 0:
      ...
      self.qs[0] = q_basin[np.argwhere(Psi_q > 0)[0][0]]
    else:
      # no-flux BC
      self.bs[0] = self.bs[1]
      self.qs[0] = self.qs[1]
  ...
  def Q_calc_advective_tendency(self, dy):
    r"""
    Compute the advective transport tendency in the Southern Ocean mixed layer:
    ...
    indneg = self.Psi_s[1:-1] < 0.
    indpos = self.Psi_s[1:-1] > 0.
    dqdt_ad[1:-1][indneg] = -self.Psi_s[1:-1][indneg] * 1e6 * (
        self.qs[2:][indneg] - self.qs[1:-1][indneg]
    ) / self.h / self.L / dy
    dqdt_ad[1:-1][indpos] = -self.Psi_s[1:-1][indpos] * 1e6 * (
        self.qs[1:-1][indpos] - self.qs[:-2][indpos]
    ) / self.h / self.L / dy
    return dqdt_a
  ...
  def Q_calc_implicit_diffusion(self, dy, dt):
    r"""
    Compute the diffusive transport in Southern Ocean mixed layer:
    ...
    return np.dot(np.dot(Uinv, V), self.qs)
  ...
  def Q_advdiff(self, q_basin, Psi_q, dt):
    r"""
    Compute and apply advective-diffusive transport to the Souther Ocean Mixed Layer model,
    based on conditions in the adjoining basin.
    ...
    # For the purpose of the interpolation to the surface we therefore set psi
    # on non-outcropping isopycnals to the last non-zero value above
    Psi_mod_q = Psi_q.copy()
    ind_q = np.nonzero(Psi_mod_q)[0][0]
    Psi_mod_q[:ind_q] = Psi_mod_q[ind_q]
